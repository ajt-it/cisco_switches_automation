...
#
        ...
        Qlines = Q.readlines()
#
	B = open('vlans_ip_addr')
	Blines = B.readlines()
#
	F = open('vlans_dhcp_pool_network')
	Flines = F.readlines()
#
        Z = open('vlans_dhcp_excluded_addr')
        Zlines = Z.readlines()
#
	d = open('vlans_number', "r")
#
	for i, line in enumerate(d):
		print "int vlan " + (line)
		tn.write("ip dhcp excluded-address " + Zlines[i] + "\n")
		tn.write("int vlan " + line + "\n")
		...
		tn.write("dns-server " + Tlines[i] + "\n")
		tn.write("domain-name " + Qlines[i] + "\n")
		tn.write("default-router " + Zlines[i] + "\n")
	d.close()
	B.close()
	F.close()
	P.close()
